...
Failure mode: The production venv is correctly upgraded to FastMCP 4.0.0b3, but every source sync rolls back after restart with `live source SHA mismatch: live=<missing>`, leaving the public MCP endpoint on the old v3.4.7 source indefinitely.
...
Use a deterministic temporary response fixture or a local endpoint test to cover:
