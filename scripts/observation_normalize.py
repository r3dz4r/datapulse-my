...
* Input: the top-level JSON array of weekly row objects served by
  ...
  manifest actually names.  ``fuelprice_csv_v1`` stays pinned for payloads
  that really are CSV: it parses this JSON as one giant CSV header line and
  fails closed on the duplicate header.  This profile reads the source as
  the JSON array it is.
* Engine: ``engine`` = ``"top-level-array"``.  The projection ``format`` is
  ``"json-array-of-objects"`` — the only JSON-array format the envelope
  schema admits — because that field names the projection, not the parser.
...
* Nulls and numbers: an explicit JSON ``null`` stays ``null`` — a null
  price is unmeasured, not free — and JSON-native types are preserved
  exactly as parsed, so integers stay integers.  No inference, no coercion.
...
#: Legacy ``format`` -> parser mapping for profiles built before ``engine``
#: existed.  ``format`` names the *projection* format the envelope can carry;
#: two profiles can share a projection format while reading different source
#: shapes, so the parser is selected by ``engine`` with this mapping as the
#: fallback.  Keeping the fallback means every already-pinned profile keeps
#: its exact behaviour.
_DEFAULT_ENGINE_BY_FORMAT: Final[Mapping[str, str]] = {
    "csv": "csv",
    "json-array-of-objects": "arcgis-features",
}
...
class NormalizationProfile:
    ...
    ``apply`` dispatches on ``engine`` (falling back to the legacy
    ``format`` mapping); every policy field is descriptive metadata that the
    pinned ``(name, version)`` pair makes auditable.
    """
    ...
    dedup: bool
    description: str
    #: Parser discriminator: ``"csv"``, ``"arcgis-features"``, or
    #: ``"top-level-array"``.  Empty defers to ``format`` so a profile built
    ...
    def apply(self, payload: bytes) -> NormalizationResult:
        ...
        engine = self.engine or _DEFAULT_ENGINE_BY_FORMAT.get(self.format, self.format)
        if engine == "csv":
            return _normalize_csv(self, payload)
        if engine == "arcgis-features":
            return _normalize_json_features(self, payload)
        if engine == "top-level-array":
            return _normalize_json_array(self, payload)
        raise NormalizationProfileError(
            f"profile {self.name}/{self.version} declares unsupported engine "
            f"{engine!r} (format {self.format!r}); no normalization was attempted"
        )
...
# ---------------------------------------------------------------------------
# Top-level-JSON-array normalization (engine for top-level-array profiles)
# ---------------------------------------------------------------------------
...
def _normalize_json_array(
    ...
    if not columns:
        raise NormalizationParseError(
            "every key is sparse (absent from at least one record); no column "
            "is present in all records, so nothing can be projected without "
            "fabricating values"
        )
...
_ROW_ORDERING_JSON_ARRAY: Final[str] = "sort_by_canonical_row_bytes_full_content_order"
...
def _run_selftest() -> int:
    ...
    try:
        ...
        check(
            "fuelprice-json: projection format is the schema-representable json array",
            fuel.format == "json-array-of-objects"
            and fuel.projection["format"] == "json-array-of-objects",
        )
