...
def render_markdown(snapshot: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## Reproducibility",
        "",
        f"Generated by `bash scripts/gen_trust_snapshot.sh`. License: MIT. Cite: {snapshot['reproducibility']['cite']}",
        "",
    ]
    return "\n".join(lines)
...
def main() -> None:
    ...
    catalog_snapshot = load_json(root / "catalog-snapshot.json")
    rows = catalog_snapshot.get("datasets", [])
    manifest = load_json(root / "datapulse.json")
    health = load_json(root / "health/latest.json")
    # The citation URL is a website surface, so the origin must come from the
    # sole public-surfaces authority rather than a hardcoded host.
    ...
    snapshot = {
        ...
        "reproducibility": {
            ...
            "license": "MIT",
            "cite": f"{website}/trust-snapshot-{snapshot_date.isoformat()}.md",
        },
    }
