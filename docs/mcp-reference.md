...
<!-- Generated by scripts/gen_mcp_reference.py from canonical local inputs. -->

DataPulse exposes 19 read-only tools over 418 datasets and the 10-status health taxonomy at
...
- `datapulse://index` — Read first; lightweight list of all DataPulse dataset ids with current status, title, source, licence, and namespace.
...
- `datapulse://reliability` — Live count of DataPulse datasets by evaluated publish-reliability grade; reliability is timeliness, not uptime.
...
- `datapulse://licences` — Live count of DataPulse datasets grouped by licence.
